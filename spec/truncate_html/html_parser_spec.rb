...
describe TruncateHtml::HtmlParser do

  def parser
    @parser ||= TruncateHtml::HtmlParser
  end
  ...
  describe '#html_tokens' do
    ...
    it 'returns each token in the string as an array element removing any consecutive whitespace from the string' do
      parser.html_tokens(@html).should == ['<h1>', 'Hi', ' ', 'there', '</h1>', ' ', '<p>', 'This', ' ', 'is', ' ', 'sweet!', '</p>']
    end
  ...
  describe '#html_tag?' do
    ...
    it 'returns false when the string parameter is not an html tag' do
      parser.html_tag?('no tags').should be_false
    end

    it 'returns true when the string parameter is an html tag' do
      parser.html_tag?('<img src="foo">').should be_true
      parser.html_tag?('</img>').should be_true
    end
  ...
  describe '#matching_close_tag' do
    it 'closes a tag given an open tag' do
      parser.matching_close_tag('<a>').should == '</a>'
      parser.matching_close_tag(' <div>').should == '</div>'
      parser.matching_close_tag('<h1>').should == '</h1>'
      parser.matching_close_tag('<a href="foo">').should == '</a>'
    end
  end
